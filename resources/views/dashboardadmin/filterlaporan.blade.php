...
<head>
    ...
    <title>Filter Laporan - LaporanKita Admin</title>
...
<body class="bg-[#FAFAFA] text-slate-900 antialiased flex h-screen overflow-hidden selection:bg-brand-500 selection:text-white">
    ...
    <aside id="sidebar" class="fixed md:static inset-y-0 left-0 w-64 bg-brand-900 text-white flex flex-col h-full z-40 sidebar-transition -translate-x-full md:translate-x-0 shrink-0 shadow-2xl md:shadow-none">
        ...
        <div class="flex-1 overflow-y-auto py-6 px-4 space-y-1.5 custom-scrollbar">
            ...
            <a href="{{ route('admin.filter') }}" class="flex items-center gap-3 px-3 py-2.5 bg-brand-800 text-white rounded-lg font-semibold text-sm transition-colors border border-brand-700 shadow-inner">
                <span class="material-symbols-outlined icon-filled text-[20px] text-brand-400">filter_alt</span>
                Filter Laporan
            </a>
        </div>
    ...
    <main class="flex-1 flex flex-col h-full w-full overflow-hidden relative">
        ...
        <header class="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-8 shrink-0 shadow-sm z-30">
            ...
            <h2 class="font-bold text-brand-900 text-lg hidden sm:block">Filter Laporan</h2>
        ...
        <div class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 custom-scrollbar">
            <div class="max-w-7xl mx-auto space-y-6">
                ...
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                    <form action="{{ route('admin.filter') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                        <div class="md:col-span-1">
                            <label class="block text-xs font-bold text-slate-400 mb-2 uppercase tracking-wide">Status</label>
                            <select name="status" class="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                        ...
                        <div class="md:col-span-1">
                            <label class="block text-xs font-bold text-slate-400 mb-2 uppercase tracking-wide">Kategori</label>
                            <select name="kategori" class="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                        ...
                        <div class="md:col-span-1">
                            <button type="submit" class="w-full bg-brand-900 text-white rounded-lg p-2.5 text-sm font-semibold hover:bg-slate-800 transition-colors shadow-md flex items-center justify-center gap-2">
                                <span class="material-symbols-outlined text-[18px]">search</span> Saring Laporan
                            </button>
                        </div>
                        <div class="md:col-span-1">
                            <a href="{{ route('admin.filter') }}" class="w-full bg-slate-100 text-slate-600 rounded-lg p-2.5 text-sm font-semibold hover:bg-slate-200 transition-colors text-center block">
                                Reset
                            </a>
                        </div>
                    </form>
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden min-h-[300px]">
                        ...
                        <table class="w-full text-left border-collapse whitespace-nowrap">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                ...
                                <tr class="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider @if($laporan->status === 'baru') text-rose-600 bg-rose-50 @elseif($laporan->status === 'diproses') text-amber-600 bg-amber-50 @else text-emerald-600 bg-emerald-50 @endif">
                                            {{ $laporan->status }}
                                        </span>
                                    </td>
                    ...
                    <div class="flex flex-col items-center justify-center p-12 text-center">
                        ...
                        <h3 class="text-lg font-bold text-slate-900">Tidak ada laporan ditemukan</h3>
                        <p class="text-sm text-slate-500 max-w-xs">Cobalah mengubah kriteria filter atau reset pencarian Anda.</p>
                    </div>
                    @endif
                </div>
