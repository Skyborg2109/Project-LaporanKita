...
<body class="bg-[#FAFAFA] text-slate-900 antialiased flex h-screen overflow-hidden selection:bg-brand-500 selection:text-white">
    ...
    <aside class="hidden md:flex flex-col w-64 h-full z-20 shrink-0 bg-gradient-to-b from-teal-600 via-brand-500 to-emerald-500 border-r-0">
        ...
        <div class="flex-1 overflow-y-auto py-6 px-4 space-y-1.5">
            ...
            <a href="{{ route('dashboarduser.laporan') }}" class="flex items-center gap-3 px-3 py-2.5 bg-white/20 backdrop-blur-sm text-white rounded-xl font-semibold text-sm shadow-sm transition-all">
                <span class="material-symbols-outlined icon-filled text-[20px]">assignment</span>
                Laporan Saya
            </a>
            ...
            <a href="{{ route('dashboarduser.notifikasi') }}" class="flex items-center gap-3 px-3 py-2.5 text-white/80 hover:bg-white/15 hover:text-white rounded-xl font-medium text-sm transition-all group relative">
                ...
                <span class="ml-auto bg-rose-500 text-white text-[10px] font-bold rounded-full px-1.5 py-0.5 min-w-[20px] text-center shadow-sm">{{ Auth::user()->unreadNotifications->count() }}</span>
                @endif
            </a>
                ...
                <div class="relative">
                    ...
                    <div id="notif-dropdown" class="absolute top-full right-0 mt-3 w-[320px] bg-white rounded-2xl shadow-2xl border border-slate-100 transition-all duration-300 opacity-0 scale-95 pointer-events-none z-[110] overflow-hidden">
                        ...
                        <div class="max-h-[350px] overflow-y-auto">
                            @forelse(Auth::user()->notifications->take(5) as $notif)
                            <div class="p-4 hover:bg-slate-50 transition-colors border-b border-slate-50 group cursor-pointer text-left">
                                <div class="flex gap-3">
                                    <div class="w-10 h-10 rounded-full @if($notif->unread()) bg-brand-50 text-brand-600 @else bg-slate-50 text-slate-400 @endif flex items-center justify-center shrink-0">
                                        <span class="material-symbols-outlined text-[18px]">@if($notif->data['type'] ?? '' === 'status') rule @else assignment_late @endif</span>
                                    </div>
