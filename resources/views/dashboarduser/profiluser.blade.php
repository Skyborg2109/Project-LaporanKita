    ...
    <aside class="hidden md:flex flex-col w-64 bg-white border-r border-slate-200 h-full z-20 shrink-0">
        ...
        <div class="p-4 border-t border-slate-100">
            <div class="flex items-center gap-3 px-3 py-2 mb-2">
                @if($user->foto_profil)
                    <img src="{{ asset('storage/' . $user->foto_profil) }}" alt="Avatar" class="w-9 h-9 rounded-full border border-slate-200 object-cover" />
                @else
                        ...
                        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden text-center p-6 relative">
                            ...
                            <form action="{{ route('dashboarduser.profil.update') }}" method="POST" enctype="multipart/form-data" id="foto-form">
                                ...
                                <div class="relative w-24 h-24 mx-auto mb-4 z-10">
                                    @if($user->foto_profil)
                                        <img id="avatar-preview" src="{{ asset('storage/' . $user->foto_profil) }}" alt="Avatar" class="w-full h-full rounded-full border-4 border-white shadow-md object-cover" />
                                    @else
